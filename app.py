...
quadratic_retail = pickle.load(open('quadratic_retail.pkl', 'rb'))
tx = pickle.load(open('tx.pkl', 'rb'))
ty = pickle.load(open('ty.pkl', 'rb'))
quadratic_corp = pickle.load(open('quadratic_corp.pkl', 'rb'))
...
def predict1():
    ...
    apartment_quad_std = quadratic_retail.transform(apartment_std)
    quadratic_price_pred = retailmodel.predict(apartment_quad_std)
    prediction=sc_y.inverse_transform(quadratic_price_pred)
...
def predict_api1():
    ...
    prediction = sc_y.inverse_transform(
        retailmodel.predict(quadratic_retail.transform(
...
def predict2():
    ...
    def get_input(df):
        ...
        for i in range(len(uz)):
            for k in range(len(df)):
                if (df.iloc[:,11][k] == uz[i]):
                    zipcode_up_column[k] = zipcode_up[i]
        df.insert(11, "zipcode_up", zipcode_up_column)
    ...
    def get_prediction(input_df):
        ...
        for i in range(len(input_df)):
            apartment = np.array(np.append(sales_time_up[0],input_df.iloc[i,:12])).reshape(1, -1)
            apartment_std = tx.transform(apartment)
            apartment_quad_std = quadratic_corp.transform(apartment_std)
            quadratic_price_pred = corpmodel.predict(apartment_quad_std)
            price_std = quadratic_price_pred
            prices[i]= ty.inverse_transform(price_std)
        
        input_df.insert(13, "estimated price", prices)
    ...
    if request.method == 'POST':
        ...
        portfolio_value =truncate((output_df.iloc[:,12].sum(axis=0)),-2)
...
def portfolio_mangement():
    ...
    def get_realestate_prediction(input_df):
        ...
        for k in range(len(sales_time_up)):
            for i in range(len(input_df)):
                apartment = np.array(np.append(sales_time_up[k],input_df.iloc[i,:12])).reshape(1, -1)
                apartment_std = tx.transform(apartment)
                apartment_quad_std = quadratic_corp.transform(apartment_std)
                quadratic_price_pred = corpmodel.predict(apartment_quad_std)
                price_std = quadratic_price_pred
...
def predict_api2():
    ...
    prediction = ty.inverse_transform(
        corpmodel.predict(quadratic_corp.transform(
