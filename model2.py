...
P = df[['sales_time_up','bedrooms', 'bathrooms', 'sqft_living','sqft_lot','floors', 'waterfront','view',
        'condition','grade','age', 'age_rnv','zipcode_up']].values
q = df['price'].values
...
# function to make prediction and then 
# return result data frame for corporate model




# create quadratic features
quadratic_corp = PolynomialFeatures(degree=2)

P_quad_train = quadratic_corp.fit_transform(P_train_std)
P_quad_test = quadratic_corp.transform(P_test_std)




#Fitting model with trainig data
Lin.fit(P_quad_train, q_train_std)

# Saving model to disk
pickle.dump(Lin, open('model2.pkl','wb'))

# Loading model to compare the results
corpmodel = pickle.load(open('model2.pkl','rb'))

print("Price $ %.3f" % ty.inverse_transform(corpmodel.predict(quadratic_corp.transform(tx.transform(np.array([284.410837,4.0,3.00,1960.0,5000.0,1.0,0.0,0.0,5.0,7.0,49.0,49.0,337.218034]).reshape(1, -1))))))
...
pickle.dump(quadratic_corp, open('quadratic_corp.pkl','wb'))
